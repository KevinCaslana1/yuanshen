...
const FREEZE = `${ROOT}/experiments/Q2_FREEZE_RUN_V3`;
...
async function main() {
  ...
  const validation = JSON.parse(await fsp.readFile(`${FREEZE}/validation.json`, "utf8"));
  const manifest = JSON.parse(await fsp.readFile(MANIFEST, "utf8"));
  const entry = manifest.entries.find((item) => item.path === "experiments/Q2_FREEZE_RUN_V3/run_1/official_samples.csv");
  if (!entry || entry.status !== "PRODUCTION_CANONICAL") throw new Error("V3 Run 1 source is not PRODUCTION_CANONICAL");
  if (validation.status !== "PRE_CANDIDATE_GATES_PASS" || Object.values(validation.gates || {}).some((value) => value !== true)) throw new Error("pre-candidate validation gates are not all PASS");
  const sourceHash = hashFile(SOURCE);
  if (sourceHash !== hashes.run_1.sampled_output || sourceHash !== entry.sha256) throw new Error("V3 Run 1 source hash does not match frozen lineage");
  if (fs.existsSync(CANDIDATE) && hashFile(CANDIDATE) !== hashFile(TEMPLATE)) throw new Error(`refusing to overwrite a non-template candidate: ${CANDIDATE}`);
  ...
  await fsp.writeFile(`${FREEZE}/candidate_preview.png`, new Uint8Array(await preview.arrayBuffer()));
