"""Run the minimum next Q4 refinement selected by the A/B/C/D audit.

The run is a fresh t=0 standalone FVM/BE/Picard calculation at n=192,
dt=2 s.  Only its isolated evidence directory is written.  The threshold
crossing step is then locally refined to 0.0625 s so root-location resolution
is not conflated with field discretization error.
"""
...
def main() -> None:
    ...
    if coarse_bracket is None:
        raise RuntimeError("n=192, dt=2 s did not cross the Q4 threshold")
    ...
    (OUT / "next_refinement_n192_dt2.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
